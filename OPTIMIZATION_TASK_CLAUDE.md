...
1. **Асинхронная префетч-задача.** Сразу после получения нормализованного текста пользователя запускать `embedding_task = asyncio.create_task(...)`, который дергает `self.agent.ensure_query_embedding(normalized_text)` через `asyncio.to_thread`. Это происходит параллельно с запуском филлера и chunked-ответа, описанных в `OptimizedAsteriskAIHandler` (см. _play_instant_filler и дальнейший вызов `get_response_generator`).
2. **Использование существующего кеша.** В `Agent.ensure_query_embedding` проверять Redis и только при отсутствии ключа синхронно вызывать `self.db._embedding_function.embed_query(text)`. Поскольку Chroma retriever использует тот же `embedding_function`, к моменту вызова `get_response_generator` он получит embedding уже из кеша без сетевого запроса.
3. **Метрики и фолбэк.** Логировать длительность подготовки embedding и при таймауте (например, >1.5с) продолжать текущий синхронный путь, чтобы не блокировать ответ.

**Ожидаемый эффект:**
- Экономия ~0.6-1.3с для новых вопросов (embedding создается во время проигрывания филлера вместо последовательного ожидания).
- Снижение средней задержки до ~2.2-2.7с даже без кеша.
- Улучшение метрики `FIRST AUDIO PLAYED`, т.к. первый чанк будет запускаться раньше.

**Технический план внедрения:**
- Добавить в `Agent` новый метод `async def ensure_query_embedding(self, text: str) -> None`, использующий `asyncio.to_thread`/`run_in_executor` для вызова `self.db._embedding_function.embed_query(text)` и логирования времени.
- В `OptimizedAsteriskAIHandler` (секция запуска филлера перед `get_response_generator`) инициализировать задачу префетча до ожидания `asyncio.sleep(0.20)` и `self.agent.get_response_generator(...)`, затем `await embedding_task` только если он ещё не завершен перед обращением к агенту.
- Добавить метрики (`embedding_prefetch_started/finished`, таймауты) в логах, чтобы подтвердить сокращение задержки.
