...
> 🪟 **Soft Window**: Динамическая обработка речи с поддержкой длинных реплик  
> 🎙️ **Barge-in через TALK_DETECT**: Мгновенное перебивание через нативные события Asterisk
...
- ✅ **Barge-in через TALK_DETECT** - мгновенное прерывание через нативные события Asterisk (ChannelTalkingStarted)
- ✅ **Динамическая обработка речи (Soft Window)** - поддержка длинных реплик до 60 секунд с параллельной обработкой
...
- ✅ **Автоматическая ротация логов** - ежедневная очистка, рост замедлен в 10 раз, диск под контролем
- ✅ **Barge-in через TALK_DETECT (08.11.2025)** - мгновенное прерывание через нативные события Asterisk без записи
- ✅ **Soft Window (07.11.2025)** - динамическая обработка речи до 60 секунд с параллельным ASR

### 🪟 **Динамическая обработка речи (Soft Window)** - ОБНОВЛЕНО 08.11.2025!

Система использует "мягкое окно" для обработки речи любой длины:

- **Запись кусками по 5 секунд БЕЗ остановки** - каждый кусок параллельно распознается
- **Префетч агента** - обработка начинается после первых кусков для сокращения задержки
- **Поддержка естественных пауз до 1.5 сек** - не обрывает при паузах в речи
- **Максимальная длина реплики - 60 сек** - общий предохранитель
- **Обратная совместимость** - можно вернуться к старому режиму через .env

### 🎙️ **Barge-in через TALK_DETECT** - НОВАЯ АРХИТЕКТУРА (08.11.2025)!

Революционный подход к детекции речи пользователя во время воспроизведения TTS:

**Как работает:**
1. **TALK_DETECT активируется в dialplan** - слушает канал без записи
2. **Детектирует речь параллельно с playback** - нет конфликта с TTS
3. **Генерирует событие `ChannelTalkingStarted`** - мгновенное уведомление
4. **Бот останавливает все playback** - через ARI DELETE с полной очисткой очередей
5. **Запускается Soft Window для записи** - после стабилизации канала (300ms)

**Преимущества перед старым подходом:**
- ✅ **Нет конфликта запись/воспроизведение** - TALK_DETECT только слушает
- ✅ **Нет ложных срабатываний** - больше не получаем пустые файлы (44 байта)
- ✅ **Мгновенная реакция** - события приходят сразу при детекции речи
- ✅ **Стабильная работа** - нет циклических пустых записей
- ✅ **Меньше нагрузки** - не нужно постоянно записывать и анализировать файлы

**Конфигурация в dialplan (`/etc/asterisk/extensions.conf`):**
```asterisk
exten => _X.,n,Set(TALK_DETECT(set)=1500,256)
```
- `1500` = тишина 1.5 сек означает конец речи
- `256` = минимальная амплитуда для детекции речи (по умолчанию из dsp.conf)

**Настройка через .env:**
```env
# Soft Window параметры
USE_SOFT_WINDOW_INPUT=true           # Включить динамическое окно
CHUNK_STREAMING_SECONDS=5            # Размер куска (сек)
MAX_UTTERANCE_SECONDS=60             # Максимальная длина реплики
VAD_SILENCE_TIMEOUT=1.5              # Таймаут тишины (согласован с TALK_DETECT)

# Barge-in параметры (устарели, но для обратной совместимости)
BARGE_IN_ENABLED=true                # Включить перебивание
BARGE_IN_GUARD_MS=800                # Защита от срабатывания сразу после TTS
```

**Защитные механизмы:**
- 🛡️ **Пропуск первых 2 playback** - игнорируем приветствие (нет ложных срабатываний от эха)
- 🛡️ **Guard period 800ms** - защита от срабатывания сразу после старта TTS
- 🛡️ **Задержка 300ms перед записью** - стабилизация канала после остановки playback
- 🛡️ **Проверка активности канала** - не запускаем запись на отключенном канале

**Подробности**: См. `docs/ENV_VARIABLES.md` - полное описание всех параметров

 ### 📊 **Метрики производительности (обновлено 07.11.2025):**
...
### 🎯 **Оптимизации выполнены (обновлено 10.10.2025):**
...
### 🔬 **Технические детали реализации Barge-in:**

**Архитектурные изменения (08.11.2025):**

1. **Dialplan (`/etc/asterisk/extensions.conf`)**:
   ```asterisk
   exten => _X.,n,Set(TALK_DETECT(set)=1500,256)
   exten => _X.,n,Stasis(asterisk-bot)
   ```
   - TALK_DETECT активируется **до** передачи в Stasis
   - Audiohook начинает слушать канал параллельно с playback

2. **WebSocket подписка (`stasis_handler_optimized.py`)**:
   ```python
   self.ws_url = "ws://localhost:8088/ari/events?app=asterisk-bot&api_key=asterisk:asterisk123&subscribeAllEvents=true"
   ```
   - `subscribeAllEvents=true` обязателен для получения `ChannelTalkingStarted`

3. **Обработчик события (`handle_talking_started`)**:
   ```python
   async def handle_talking_started(self, event):
       # 1. Валидация канала и playback_count
       # 2. Guard period 800ms от старта TTS
       # 3. Остановка всех playback через stop_tts_on_barge_in_optimized
       # 4. Задержка 300ms для стабилизации
       # 5. Запуск Soft Window для записи речи
   ```

4. **Удаленная логика (больше не нужна)**:
   - ❌ `_start_barge_in_recording()` - параллельная запись во время TTS
   - ❌ `_on_barge_in_recording_finished()` - обработка пустых файлов
   - ❌ Проверки размера файла (MIN_FILE_SIZE, MIN_DURATION)

**Почему старый подход не работал:**
```
Asterisk НЕ МОЖЕТ одновременно:
┌──────────────────────────────┐
│ Воспроизводить (playback)    │  ← TTS говорит
│            И                  │
│ Записывать (record)          │  ← Пытаемся детектировать речь
└──────────────────────────────┘
         ↓
    Результат: пустой файл (44 байта = WAV header)
         ↓
    Ложное срабатывание barge-in
         ↓
    Циклические пустые записи
```

**Как работает новый подход:**
```
TALK_DETECT использует audiohook:
┌──────────────────────────────┐
│ Воспроизводить (playback)    │  ← TTS говорит
│            И                  │
│ Слушать через audiohook      │  ← TALK_DETECT анализирует
└──────────────────────────────┘
         ↓
    Генерирует событие ChannelTalkingStarted
         ↓
    Мгновенная остановка playback
         ↓
    Запуск ЧИСТОЙ записи (без конфликта)
```

### 🔧 **Экспериментальные функции (в коде, но не активны):**
- 🔧 **Filler words** - генерируются и кешируются (`filler_tts.py`), но не озвучиваются

> 📝 **Примечание:** Barge-in через запись был полностью заменен на TALK_DETECT события. Старый код удален из проекта (08.11.2025). Озвучивание филлеров протестировано, но откачено из-за нестабильности.
...
### **15.10.2025 - v2.3 (Исправление ротации и освобождение диска)**
- ✅ **Исправлена ротация Asterisk логов** - удалено 10GB старых логов
- ✅ **Уменьшен уровень логирования** - убраны NOTICE записи о SIP-атаках
- ✅ **Рост логов замедлен в 10 раз** - с 1.5GB/день до 150MB/день
- ✅ **Исправлена ротация Metrotech** - добавлен copytruncate для systemd
- ✅ **Создан CHECK_DISK/** - инструменты мониторинга дискового пространства
- ✅ **Диск освобожден**: 83% → 47% (освобождено ~10GB)
- ✅ **Полная документация** - `CHECK_DISK/README.md` и `CHANGELOG.md`
...
> 📖 **Новичкам**: Подробный гайд по логированию с объяснениями для новичков находится в `docs/LOGGING_GUIDE.md`  
> 💾 **Мониторинг диска**: Гайд по проверке дискового пространства находится в `../CHECK_DISK/README.md`
...
> 📖 **Подробный гайд по логированию**: См. `docs/LOGGING_GUIDE.md` - полное руководство для новичков  
> 💾 **Мониторинг дискового пространства**: См. `../CHECK_DISK/README.md` - автоматическая проверка и очистка

---

**Дата создания**: 2025-01-27  
**Версия**: 2.3  
**Последнее обновление**: 15.10.2025 (Исправление ротации и освобождение диска)  
**Статус**: ✅ **Защищен, оптимизирован и работает стабильно**
